  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        <BimStat icon={<Layers   size={16} className="text-blue-500" />}   label="Bruksareal"      value={`${project.bra.toLocaleString('no')} m²`} />
        <BimStat icon={<Building2 size={16} className="text-violet-500" />} label="Enheter"         value={project.units.toString()} />
        <BimStat icon={<Box      size={16} className="text-amber-500" />}   label="Etasjer"         value={project.floors.toString()} />
        <BimStat icon={<Calendar size={16} className="text-emerald-500" />} label="Ferdigstillelse" value={project.completionYear.toString()} />
        <BimStat icon={<TrendingUp size={16} className="text-rose-500" />}  label="Arealformål"     value={project.zoningCode} />
      </div>

      {/* Zoning */}
      <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl px-4 py-3 mb-6 flex items-center gap-3">
      ...
      <div className="grid grid-cols-2 gap-5 mb-6">
        <div>
          ...
          <div className="space-y-2">
            <ScoreCard score={project.analysis.sol.score}           label={project.analysis.sol.label}           description={project.analysis.sol.description}           icon={<Sun      size={14} />} />
            <ScoreCard score={project.analysis.støy.score}          label={project.analysis.støy.label}          description={project.analysis.støy.description}          icon={<Volume2  size={14} />} />
            <ScoreCard score={project.analysis.flom.score}          label={project.analysis.flom.label}          description={project.analysis.flom.description}          icon={<Droplets size={14} />} />
            <ScoreCard score={project.analysis.fjernvirkning.score} label={project.analysis.fjernvirkning.label} description={project.analysis.fjernvirkning.description} icon={<Eye      size={14} />} />
          </div>
        </div>
